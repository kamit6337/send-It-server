...
const makeLoginUser = catchGraphQLError(async (parent, args, contextValue) => {
  ...
  if (!email || !password) {
    throw new Error("Email or Password is not provided", 404);
  }

  const findUser = await getUserByEmail(email);

  if (!findUser) {
    throw new Error("Email or Password is incorrect", 404);
  }

  if (!findUser.password) {
    throw new Error("Please reset your password to login", 404);
  }
